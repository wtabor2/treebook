...
class StatusesControllerTest < ActionController::TestCase
  ...
  test "should be logged in to post statues" do
  ...
  test "should create status for current user when logged in" do
    ...
    assert_redirected_to status_path(assigns(:status))
    assert_equal assigns(:status).user_id, users(:ren).id
  end
  ...
  test "should update status for the current user when logged in" do
    sign_in users(:ren)
    put :update, id: @status, status: { content: @status.content, user_id: users(:jackson).id}
    assert_redirected_to status_path(assigns(:status))
    assert_equal assigns(:status).user_id, users(:ren).id
  end
